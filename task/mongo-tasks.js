...
async function task_1_4(db) {
		const temp_res = await db.collection('orders').aggregate([
        ...
        },
        {
            $project: {
                _id: 0,
                "Customer Id": "$_id",
                "Total number of Orders": { $round: ["$specount", 3]},
                "% of all orders" : { $round: [ { $multiply: [ { $divide:[ "$specount", temp_res[0].count ]}, 100 ]}, 3]}
            }
        },
...
async function task_1_13(db) {
	const temp_res = await db.collection('products').aggregate([
			{$group: { _id: "$Discontinued", count: {$sum: 1}}},
			{$project: { _id: 0, count: "$count"}},
			{$sort: {count: -1}}
		]).toArray();
		
	const result = {"TotalOfCurrentProducts": temp_res[0].count + temp_res[1].count, "TotalOfDiscontinuedProducts": temp_res[1].count};
    return result;
}
...
async function task_1_15(db) {
	const temp_res = await db.collection('orders').aggregate([
			...
			{$group: { _id: {$month: {$dateFromString: {dateString: "$OrderDate"}}}, count: {$sum: 1}}},
			{$project: { _id: 0, month:"$_id",  count: "$count"}},
			{$sort: {month: 1}}
		]).toArray();
		
	const result = {"January": temp_res[0].count, 
					"February": temp_res[1].count,
					"March": temp_res[2].count, 
					"April": temp_res[3].count,
					"May": temp_res[4].count, 
					"June": temp_res[5].count,
					"July": temp_res[6].count, 
					"August": temp_res[7].count,
					"September": temp_res[8].count, 
					"October": temp_res[9].count,
					"November": temp_res[10].count, 
					"December": temp_res[11].count,};
    return result;
}
...
async function task_1_18(db) {
    const result = await db.collection('orders').aggregate([
			...
			{$group: { _id: {$dateFromString: {dateString: "$OrderDate"}},  count: {$sum: 1}}},
			{$project: { _id: 0, "Order Date": {$dateToString: {date:"$_id", format: "%Y-%m-%d"} }, "Total Number of Orders": "$count"}},
			{$sort: {"Order Date": 1}}
		]).toArray();
...
async function task_1_21(db) {
		...
		{ 
			$project: { 
				_id: 0, 
				"OrderID": "$_id",
				"Maximum Purchase Amount, $": {$round: ["$amount", 2]},
			}	
		},
